...
function buildSchemaCatalog(migrationSql: string): SchemaCatalog {
  ...
  // INTENTIONALLY-UNUSED markers: find every marker position, then scan
  // the text between consecutive markers (or to EOF) for schema objects.
  // This avoids block-boundary regex issues with comment lines.
  const markerRe = /--\s*INTENTIONALLY-UNUSED/gi;
  const markers: number[] = [];
  while ((m = markerRe.exec(migrationSql))) {
    markers.push(m.index);
  }
  for (let i = 0; i < markers.length; i++) {
    const start = markers[i];
    const end = markers[i + 1] ?? migrationSql.length;
    const block = migrationSql.substring(start, end).replace(/--[^\n]*/g, "");
    const tableInBlock = /CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?([A-Za-z0-9_."]+)\s*\(/i.exec(block);
    const fnInBlock = /CREATE\s+(?:OR\s+REPLACE\s+)?FUNCTION\s+([A-Za-z0-9_."]+)\s*\(/i.exec(block);
    const bucketRe = /INSERT\s+INTO\s+storage\.buckets[^;]*VALUES\s*\(/gi;
    const bucketInBlock = bucketRe.exec(block);
    ...
    if (bucketInBlock) {
      // Extract the bucket name from VALUES
      const rest = block.substring(bucketRe.lastIndex);
      const bname = /['"]([^'"]+)['"]/i.exec(rest);
      if (bname) intentionallyUnusedBuckets.add(bname[1]);
    }
  }
