...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 16, paddingBottom: 12, borderBottomWidth: 2, borderBottomColor: "#D97706" },
  schoolName: { fontSize: 16, fontWeight: "bold", color: "#0B1220" },
  schoolAddress: { fontSize: 8, color: "#999", marginTop: 2 },
  title: { fontSize: 13, fontWeight: "bold", textAlign: "center", marginBottom: 16, color: "#0B1220", textTransform: "uppercase", letterSpacing: 1.5 },
  ...
  tableHeader: { backgroundColor: "#0B1220", color: "#fff", fontWeight: "bold" },
    ...
    <Document>
      <Page size="A4" orientation="landscape" style={styles.page}>
            ...
            <View key={rowIdx} style={[styles.tableRow, rowIdx % 2 === 0 ? { backgroundColor: "#f9f9f9" } : {}]}>
              {selectedFields.map((field, colIdx) => {
                ...
                if (field.select.includes(".")) {
                  value = getNestedValue(row as Record<string, unknown>, field.select);
                } else {
        ...
        <View style={styles.footer}>
          ...
          <Text render={({ pageNumber, totalPages }: { pageNumber: number; totalPages: number }) => `Page ${pageNumber} of ${totalPages}`} fixed />
        </View>
      </Page>
    </Document>
  );
