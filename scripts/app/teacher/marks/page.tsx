...
export default function TeacherMarksPage() {
  ...
  // Get existing marks
  const { data: existingMarks = [] } = useQuery<ExistingMark[]>({
  ...
  useEffect(() => {
    ...
    const marksMap = new Map(existingMarks.map((m: ExistingMark) => [m.student_id, m]));
    setMarks(
      students.map((e: EnrollmentWithStudent) => {
  ...
  const autoSaveMark = useCallback(
    async (mark: MarkEntry) => {
      ...
      try {
        const { data: { user } } = await supabase.auth.getUser();
        const markData: TablesInsert<'marks'> = {
          school_id: school.id,
