...
import { CheckCircle2, XCircle, Clock, Loader2, AlertCircle, WifiOff, RefreshCw } from 'lucide-react';
...
export default function TeacherAttendancePage() {
  ...
  const { data: assignments = [] } = useQuery<Assignment[]>({
    ...
    queryFn: async () => {
      ...
      return (json.data?.classes || []).map((c: { classId: string; className: string; stream: string | null }) => ({
        class_id: c.classId,
        subject_id: null,
  ...
  // Derived class ID — auto-selects first homeroom class when data loads
  const resolvedClassId = selectedClassId || homeroomClasses[0]?.class_id || '';

  // Load students + existing attendance
  const { data: queryResult, isLoading: loadingStudents } = useQuery<{
    students: StudentEntry[];
    initialAttendance: Map<string, AttendanceStatus>;
  }>({
    queryKey: ['attendance-students', resolvedClassId, selectedDate, currentTerm?.id],
    queryFn: async () => {
      ...
      const list: StudentEntry[] = (enrollments || []).map((e: { student_id: string; students: { id: string; full_name: string; admission_number: string } | null }) => ({
      ...
      return { students: list, initialAttendance };
    },
    enabled: !!resolvedClassId && !!currentTerm?.id,
  });
  ...
  useEffect(() => {
    if (queryResult) {
      setLocalAttendance(queryResult.initialAttendance);
    }
  }, [queryResult]);
  ...
  const handleSubmitAll = async () => {
    ...
    const className = homeroomClasses.find((c) => c.class_id === resolvedClassId)?.class?.name || 'Unknown';
