  ...
  const studentsQuery = useQuery({
    ...
    queryFn: async (): Promise<{ students: StudentRow[]; total: number }> => {
      let query = supabase
        .from("students")
        .select(
          "id, school_id, full_name, gender, photo_url, admission_number, parent_name, parent_phone, enrollment_date, status, current_class_id, current_class:classes(id, name)",
          { count: "exact" }
        )
      ...
      const enriched: StudentRow[] = (data || []).map((s) => {
        ...
        return {
          ...student,
          fee_account: feeMap[student.id] || null,
        };
      });
