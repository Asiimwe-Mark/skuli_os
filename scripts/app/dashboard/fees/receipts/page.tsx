...
export default function ReceiptsPage() {
  ...
  // ?"EUR?"EUR Export ?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR
  ...
  return (
    ...
    >
              ...
              <tbody>
                {filtered.map((payment, i) => {
                  ...
                  return (
                    ...
                    >
                      <td className="px-4 py-3" data-label="Receipt No.">
                      ...
                      <td className="px-4 py-3 text-sm text-right font-semibold text-success-700" data-label="Amount">
                      ...
                      <td className="px-4 py-3 text-sm text-disabled" data-label="Date">
        ...
        <DialogContent className="max-w-md print:max-w-full print:shadow-none print:border-0">
          {selectedPayment && (
            <div id="receipt-content" className="space-y-6 print:text-black">
              {/* School Header */}
              <div className="text-center border-b border-border print:border-border-strong pb-4">
                {school?.logo_url && (
                  <img
                    src={school.logo_url}
                    alt=""
                ...
                {school?.address && (
                  <p className="text-xs text-disabled print:text-muted">
                    {school.address}
                  </p>
                )}
                {school?.phone && (
              ...
              <div className="text-center">
                <h3 className="text-sm font-semibold text-text-secondary print:text-warning-700 uppercase tracking-wider">
                  Payment Receipt
                </h3>
              ...
              {/* Student Info */}
              <div className="bg-bg-tertiary print:bg-bg-tertiary rounded-lg p-4 space-y-2">
                <div className="flex justify-between text-sm">
                  <span className="text-disabled print:text-muted">Student Name</span>
                  <span className="font-medium">
                    {selectedPayment.student?.full_name}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-disabled print:text-muted">Admission No.</span>
                  <span className="font-mono">
                    {selectedPayment.student?.admission_number}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-disabled print:text-muted">Class</span>
                  <span>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span className="text-disabled print:text-muted">Amount Paid</span>
                  <span className="text-lg font-bold text-text-heading print:text-success-700">
                    {formatUGX(selectedPayment.amount)}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-disabled print:text-muted">Payment Method</span>
                  <span>{methodLabels[selectedPayment.payment_method]}</span>
                </div>
                {selectedPayment.payment_method === "mobile_money" && (
                  <>
                    <div className="flex justify-between text-sm">
                      <span className="text-disabled print:text-muted">Provider</span>
                      <span>
                    ...
                    {selectedPayment.mobile_money_transaction_id && (
                      <div className="flex justify-between text-sm">
                        <span className="text-disabled print:text-muted">Transaction ID</span>
                        <span className="font-mono">
                          {selectedPayment.mobile_money_transaction_id}
                        </span>
                      </div>
                    ...
                    {selectedPayment.phone_used && (
                      <div className="flex justify-between text-sm">
                        <span className="text-disabled print:text-muted">Phone</span>
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-disabled print:text-muted">Date</span>
              ...
              {selectedPayment.fee_account && (
                <div className="bg-bg-tertiary print:bg-bg-tertiary rounded-lg p-4 space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-disabled print:text-muted">Total Expected</span>
                    <span>
                      {formatUGX(selectedPayment.fee_account.total_expected)}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-disabled print:text-muted">Total Paid</span>
                    <span className="text-text-heading print:text-success-700">
                      {formatUGX(selectedPayment.fee_account.total_paid)}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm font-bold border-t border-border print:border-border-strong pt-2">
                    <span>Balance</span>
                    <span
                      className={
              ...
              {selectedPayment.received_by && (
                <div className="flex justify-between text-sm">
                  <span className="text-disabled print:text-muted">Received By</span>
              ...
              {selectedPayment.notes && (
                <div className="text-sm">
                  <span className="text-disabled print:text-muted">Notes: </span>
              ...
              <div className="flex justify-center pt-2">
                <div className="text-center">
                  <QRCode
                    ...
                    bgColor="transparent"
                  />
                  <p className="text-[10px] text-muted mt-1">Scan to verify</p>
                </div>
              </div>

              {/* Footer */}
              <div className="text-center text-xs text-muted print:text-disabled pt-4 border-t border-border print:border-border-strong">
      ...
      {/* Print styles */}
      <style jsx global>{`@media print { /* Hide everything except receipt */ body > div, body > nav, body > header, body > aside, [data-radix-portal], .fixed { display: none !important; } body * { visibility: hidden; } #receipt-content, #receipt-content * { visibility: visible; } #receipt-content { position: absolute; left: 0; top: 0; width: 100%; max-width: 100%; padding: 2rem; background: white !important; color: black !important; } #receipt-content .text-secondary { color: #92400e !important; } #receipt-content .text-secondary { color: #047857 !important; } #receipt-content .text-secondary { color: #be123c !important; } #receipt-content .text-disabled, #receipt-content .text-muted { color: #6b7280 !important; } #receipt-content .bg-bg-tertiary { background-color: #f3f4f6 !important; } #receipt-content .border-border { border-color: #d1d5db !important; } #receipt-content .print\\:hidden { display: none !important; } /* Hide the dialog overlay and portal */ [data-state="open"] { position: static !important; background: none !important; backdrop-filter: none !important; } }`}</style>
    </motion.div>
  );
}
