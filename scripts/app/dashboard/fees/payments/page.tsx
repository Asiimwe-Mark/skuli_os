...
export default function AllPaymentsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (filterMethod !== "all") query = query.eq("payment_method", filterMethod as any);
      ...
      const rows = ((data ?? []) as unknown[]).map((p) => {
        const payment = p as Record<string, unknown>;
        return {
          ...payment,
          student: Array.isArray(payment.student) ? payment.student[0] : payment.student,
          received_by: Array.isArray(payment.received_by) ? payment.received_by[0] : payment.received_by,
          fee_account: Array.isArray(payment.fee_account) ? payment.fee_account[0] : payment.fee_account,
        } as PaymentRow;
      });
