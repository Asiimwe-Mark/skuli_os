...
export default function FeeReportsPage() {
  ...
  const paymentsQuery = useQuery({
    queryKey: ['fees-reports', 'payments', school?.id, currentTerm?.id, termStart, termEnd],
    enabled: !!school?.id,
    queryFn: async (): Promise<PaymentWithJoins[]> => {
      ...
      return ((data || []) as unknown as PaymentWithJoins[]).map((p) => ({
        ...p,
  ...
  const feeAccountsQuery = useQuery({
    queryKey: ['fees-reports', 'fee-accounts', school?.id],
    enabled: !!school?.id,
    queryFn: async (): Promise<FeeAccountWithStudent[]> => {
      const supabase = useSupabaseBrowser();
      const { data, error } = await supabase
        .from('fee_accounts')
        ...
        .eq('is_deleted', false);
      if (error) throw error;
      return (data || []) as FeeAccountWithStudent[];
    },
  });

  const classesQuery = useQuery({
    queryKey: ['fees-reports', 'classes', school?.id],
    enabled: !!school?.id,
  ...
  const feeAccounts = feeAccountsQuery.data ?? [];
  const classes = classesQuery.data ?? [];
  const loading = paymentsQuery.isLoading || feeAccountsQuery.isLoading || classesQuery.isLoading;
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3"><Skeleton className="h-10 w-full sm:w-[150px]" /><Skeleton className="h-10 w-full sm:w-[150px]" /></div>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div>
        ...
        <p className="text-sm text-disabled">Fee collection reports and analytics</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-muted text-sm hidden sm:inline">to</span>
      ...
      <Card className="bg-card">
        <CardContent className="p-5">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-disabled">Total Collected</p>
              <p className="text-2xl font-bold text-success-700">{formatUGX(totalCollected)}</p>
              <p className="text-xs text-muted mt-1">{filteredPayments.length} payments</p>
            </div>
            <div className="p-3 rounded-lg bg-success-50"><Wallet className="w-6 h-6 text-success-700" /></div>
          </div>
        </CardContent>
      </Card>
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="bg-bg-tertiary border-b border-border">
                          <th className="px-4 py-3 text-left text-xs font-medium text-disabled uppercase">Date</th>
                          <th className="px-4 py-3 text-right text-xs font-medium text-disabled uppercase">Mobile Money</th>
                          <th className="px-4 py-3 text-right text-xs font-medium text-disabled uppercase">Cash</th>
                          <th className="px-4 py-3 text-right text-xs font-medium text-disabled uppercase">Bank</th>
                          <th className="px-4 py-3 text-right text-xs font-medium text-disabled uppercase">Waiver</th>
                          <th className="px-4 py-3 text-right text-xs font-medium text-disabled uppercase">Total</th>
                          <th className="px-4 py-3 text-right text-xs font-medium text-disabled uppercase">Count</th>
                        </tr>
                      </thead>
                      <tbody>
                        {dailyCollections.map((dc) => (
                          <tr key={dc.date} className="border-b border-border bg-bg-tertiary hover:bg-card-hover transition-colors">
                            <td className="px-4 py-3 text-sm font-medium">{formatDate(dc.date)}</td>
                            <td className="px-4 py-3 text-sm text-right text-text-heading">{dc.mobile_money > 0 ? formatUGX(dc.mobile_money) : '—'}</td>
                            <td className="px-4 py-3 text-sm text-right text-text-heading">{dc.cash > 0 ? formatUGX(dc.cash) : '—'}</td>
                            <td className="px-4 py-3 text-sm text-right text-text-heading">{dc.bank > 0 ? formatUGX(dc.bank) : '—'}</td>
                            <td className="px-4 py-3 text-sm text-right text-text-heading">{dc.waiver > 0 ? formatUGX(dc.waiver) : '—'}</td>
                            <td className="px-4 py-3 text-sm text-right font-bold">{formatUGX(dc.total)}</td>
                            <td className="px-4 py-3 text-sm text-right text-disabled">{dc.count}</td>
                          </tr>
                        ))}
                      </tbody>
                      <tfoot>
                        <tr className="bg-bg-tertiary border-t border-border font-bold">
                          ...
                          <td className="px-4 py-3 text-sm text-right text-text-heading">{formatUGX(dailyCollections.reduce((s, d) => s + d.mobile_money, 0))}</td>
                          <td className="px-4 py-3 text-sm text-right text-text-heading">{formatUGX(dailyCollections.reduce((s, d) => s + d.cash, 0))}</td>
                          <td className="px-4 py-3 text-sm text-right text-text-heading">{formatUGX(dailyCollections.reduce((s, d) => s + d.bank, 0))}</td>
                          <td className="px-4 py-3 text-sm text-right text-text-heading">{formatUGX(dailyCollections.reduce((s, d) => s + d.waiver, 0))}</td>
                          <td className="px-4 py-3 text-sm text-right">{formatUGX(dailyCollections.reduce((s, d) => s + d.total, 0))}</td>
                          <td className="px-4 py-3 text-sm text-right text-disabled">{dailyCollections.reduce((s, d) => s + d.count, 0)}</td>
                        </tr>
                      </tfoot>
                    </table>
                  </div>
          ...
          <Card className="bg-card">
            ...
            <CardContent>
              ...
              {classCollections.length === 0 ? (
                <div className="text-center py-12 text-muted"><Users className="w-10 h-10 mx-auto mb-2 opacity-50" /><p>No class data available</p></div>
              ) : (
                <div className="rounded-xl border border-border overflow-hidden">
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      <thead><tr className="bg-bg-tertiary border-b border-border">
                        <th className="px-4 py-3 text-left text-xs font-medium text-disabled uppercase">Class</th>
                        <th className="px-4 py-3 text-right text-xs font-medium text-disabled uppercase">Students</th>
                        <th className="px-4 py-3 text-right text-xs font-medium text-disabled uppercase">Expected</th>
                        <th className="px-4 py-3 text-right text-xs font-medium text-disabled uppercase">Collected</th>
                        <th className="px-4 py-3 text-right text-xs font-medium text-disabled uppercase">Outstanding</th>
                        <th className="px-4 py-3 text-right text-xs font-medium text-disabled uppercase">Rate</th>
                      </tr></thead>
                      <tbody>
                        {classCollections.map((cc) => (
                          <tr key={cc.class_name} className="border-b border-border bg-bg-tertiary hover:bg-card-hover transition-colors">
                            <td className="px-4 py-3 text-sm font-medium">{cc.class_name}</td>
                            <td className="px-4 py-3 text-sm text-right text-disabled">{cc.student_count}</td>
                            <td className="px-4 py-3 text-sm text-right">{formatUGX(cc.total_expected)}</td>
                            <td className="px-4 py-3 text-sm text-right text-success-700">{formatUGX(cc.total_paid)}</td>
                            <td className="px-4 py-3 text-sm text-right text-danger-700">{formatUGX(cc.outstanding)}</td>
                      ...
                      <tfoot><tr className="bg-bg-tertiary border-t border-border font-bold">
                        <td className="px-4 py-3 text-sm">Total</td>
                        <td className="px-4 py-3 text-sm text-right text-disabled">{classCollections.reduce((s, c) => s + c.student_count, 0)}</td>
                        <td className="px-4 py-3 text-sm text-right">{formatUGX(classCollections.reduce((s, c) => s + c.total_expected, 0))}</td>
                        <td className="px-4 py-3 text-sm text-right text-success-700">{formatUGX(classCollections.reduce((s, c) => s + c.total_paid, 0))}</td>
                        <td className="px-4 py-3 text-sm text-right text-danger-700">{formatUGX(classCollections.reduce((s, c) => s + c.outstanding, 0))}</td>
          ...
          <Card className="bg-card">
            ...
            <CardContent>
              {staffCollections.length === 0 ? (
                <div className="text-center py-12 text-muted"><Users className="w-10 h-10 mx-auto mb-2 opacity-50" /><p>No payment data available</p></div>
              ) : (
                <div className="rounded-xl border border-border overflow-hidden">
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      <thead><tr className="bg-bg-tertiary border-b border-border">
                        <th className="px-4 py-3 text-left text-xs font-medium text-disabled uppercase">Staff Name</th>
                        <th className="px-4 py-3 text-right text-xs font-medium text-disabled uppercase">Payments</th>
                        <th className="px-4 py-3 text-right text-xs font-medium text-disabled uppercase">Total Collected</th>
                        <th className="px-4 py-3 text-right text-xs font-medium text-disabled uppercase">% of Total</th>
                      </tr></thead>
                      <tbody>
                        {staffCollections.map((sc) => (
                          <tr key={sc.user_id} className="border-b border-border bg-bg-tertiary hover:bg-card-hover transition-colors">
                            <td className="px-4 py-3 text-sm font-medium">{sc.name}</td>
                            <td className="px-4 py-3 text-sm text-right text-disabled">{sc.payment_count}</td>
                            <td className="px-4 py-3 text-sm text-right font-semibold text-success-700">{formatUGX(sc.total_collected)}</td>
                            <td className="px-4 py-3 text-sm text-right text-disabled">{totalCollected > 0 ? `${Math.round((sc.total_collected / totalCollected) * 100)}%` : '—'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
