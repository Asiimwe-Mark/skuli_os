...
export default function ExpenseCategoriesPage() {
  // Audit (Bug #4): scope query keys by school.id so category edits
  // in school A don't accidentally invalidate school B's cache.
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      if (school?.id) {
        queryClient.invalidateQueries({ queryKey: queryKeys.expenseCategories(school.id) });
        // Cascade: changing a category affects the expenses list.
        queryClient.invalidateQueries({ queryKey: queryKeys.expenses(school.id) });
      }
      setDialogOpen(false);
      setName('');
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-heading mt-1">Manage expense categories for your school</p>
        </div>
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              <Tag className="w-12 h-12 mx-auto text-heading mb-3" />
              <p className="text-heading">No categories yet</p>
              <p className="text-sm text-heading mt-1">Add your first expense category to get started</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
