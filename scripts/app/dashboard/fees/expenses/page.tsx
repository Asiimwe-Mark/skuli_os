...
import { fetchArray } from '@/lib/api-fetch';
...
export default function ExpensesPage() {
  // Audit (Bug #4): scope every cache key by school.id. The previous
  // keys were prefix-only — a SUPER_ADMIN switching schools would see
  // the previous school's data. Also gate the queries on school being
  // loaded so we never run with `key = ['expenses', undefined]`.
  ...
  const { data: expenses, isLoading } = useQuery({
    queryKey: queryKeys.expenses(school?.id ?? '', currentTerm?.id),
    queryFn: async () => {
      const params = new URLSearchParams();
      if (currentTerm?.id) params.set('term_id', currentTerm.id);
      return fetchArray<Expense>(`/api/fees/expenses?${params}`);
    },
    enabled: !!school?.id && !!currentTerm?.id,
  });

  const { data: categories } = useQuery({
    queryKey: queryKeys.expenseCategories(school?.id ?? ''),
    queryFn: () => fetchArray<Category>('/api/fees/expenses/categories'),
    enabled: !!school?.id,
  });

  const { data: payments } = useQuery({
    queryKey: queryKeys.feePaymentsIncome(school?.id ?? '', currentTerm?.id),
    queryFn: () => fetchArray<any>(`/api/fees/payments?term_id=${currentTerm?.id}`),
    enabled: !!school?.id && !!currentTerm?.id,
  });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      // Audit (Bug #5): use the centralised invalidator so the
      // expense, fees-index, and dashboard queries all refresh in
      // lock-step. Previously this only invalidated the local
      // ['expenses'] prefix, which matched every school's cache.
      if (school?.id) {
        invalidate.expenseChanged(queryClient, school.id);
      }
      setDialogOpen(false);
      resetForm();
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-heading mt-1">Track and analyze school expenses</p>
        </div>
        <div className="flex gap-2">
          ...
          <Button variant="outline" onClick={handleExportPL} className="bg-bg-tertiary text-white hover:bg-card-hover">
          ...
          <Card className="bg-card">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-heading">Total Income</p>
                  <p className="text-2xl font-bold mt-1 text-success-700">{formatUGX(totalIncome)}</p>
                  <p className="text-xs text-heading mt-1">Fee payments this term</p>
                </div>
                <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-success-50">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <Card className="bg-card">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-heading">Total Expenses</p>
                  <p className="text-2xl font-bold mt-1 text-danger-600">{formatUGX(totalExpenses)}</p>
                  <p className="text-xs text-heading mt-1">This term</p>
                </div>
                <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-danger-50">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          <Card className="bg-card">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-heading">Net {netSurplus >= 0 ? 'Surplus' : 'Deficit'}</p>
                  <p className={cn('text-2xl font-bold mt-1', netSurplus >= 0 ? 'text-success-700' : 'text-danger-600')}>
                    {formatUGX(Math.abs(netSurplus))}
                  </p>
                  <p className="text-xs text-heading mt-1">{netSurplus >= 0 ? 'Surplus' : 'Deficit'} this term</p>
                </div>
                <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-warning-50">
                  <Wallet className="w-6 h-6 text-warning-700" />
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <Card>
          <CardContent className="p-6">
            ...
            {weeklyChartData.length > 0 ? (
                ...
                <BarChart data={weeklyChartData}>
                  ...
                  <YAxis stroke="#666" fontSize={11} tickFormatter={(v) => `${(v / 1000000).toFixed(1)}M`} />
                    <RechartsTooltip
                      ...
                      formatter={(value) => formatUGX(value as number | string | null | undefined)}
                    />
              ...
              <div className="h-[250px] flex items-center justify-center text-heading">
        ...
        <Card>
          <CardContent className="p-6">
            ...
            {categoryChartData.length > 0 ? (
              <>
                ...
                <div className="mt-2 space-y-1">
                  {categoryChartData.map((entry, i) => (
                    <div key={entry.name} className="flex items-center gap-2 text-xs text-heading">
              ...
              <div className="h-[200px] flex items-center justify-center text-heading">
                No expenses yet
              </div>
            )}
          </CardContent>
                ...
                <TableBody>
                  {paginatedExpenses.map((expense: Expense, i: number) => (
                    ...
                    >
                      ...
                      <TableCell className="text-right text-text-heading font-medium">
