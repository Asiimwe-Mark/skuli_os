...
export default function BillingPage() {
  ...
  const supabase = useMemo(() => getBillingClient(), []);
  ...
  useEffect(() => {
    ...
    if (paymentStatus === "success") {
      ...
      // Refresh school data
      if (school) {
        getBillingClient().from("schools").select("id, name, subscription_plan, subscription_status, trial_ends_at, school_code, address, district, phone, email, logo_url, motto, school_type").eq("id", school.id).single().then(({ data }) => {
          if (data) setSchool({ ...school, ...data } as any);
        });
  ...
  }, [searchParams, school, setSchool, router, toast, queryClient]);
