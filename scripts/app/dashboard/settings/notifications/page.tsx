  ...
  const { data: prefs, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select("id, school_id, sms_enabled, send_receipt_sms, send_absence_sms, send_weekly_defaulter, defaulter_reminder_day, defaulter_reminder_hour, send_report_card_sms, send_term_start_sms")
      ...
      if (!data) {
        ...
        const { data: created, error: createErr } = await supabase
          .from("notification_preferences")
          .insert({ school_id: school!.id } as any)
          .select()
          .single();
      ...
      <motion.div {...fadeInUp} transition={{ delay: 0.05 }}>
        <Card className="bg-card">
          ...
          <CardContent className="space-y-6">
            {/* Receipt SMS */}
            <div className="flex items-start justify-between gap-4 p-4 rounded-lg bg-bg-tertiary">
              <div className="flex items-start gap-3">
                <div className="w-9 h-9 rounded-lg bg-success-100 flex items-center justify-center shrink-0 mt-0.5">
                  <MessageSquare className="w-4 h-4 text-success-700" />
                </div>
                <div>
                  <p className="font-medium text-heading">Send receipt SMS on payment</p>
            ...
            <div className="flex items-start justify-between gap-4 p-4 rounded-lg bg-bg-tertiary">
              <div className="flex items-start gap-3">
                <div className="w-9 h-9 rounded-lg bg-danger-100 flex items-center justify-center shrink-0 mt-0.5">
                  <AlertTriangle className="w-4 h-4 text-danger-700" />
                </div>
                <div>
                  <p className="font-medium text-heading">Send absent alert SMS to parents</p>
                  <p className="text-sm text-heading mt-0.5">
            ...
            {/* Defaulter Reminder */}
            <div className="p-4 rounded-lg bg-bg-tertiary space-y-4">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-medium text-heading">Send weekly defaulter reminder</p>
                    <p className="text-sm text-heading mt-0.5">
              ...
              {prefs.send_weekly_defaulter && (
                ...
                >
                  <div className="space-y-2">
                    <Label className="text-xs text-heading">Day of Week</Label>
                    <Select
                  ...
                  <div className="space-y-2">
                    <Label className="text-xs text-heading">Hour</Label>
                    <Select
            ...
            <div className="flex items-start justify-between gap-4 p-4 rounded-lg bg-bg-tertiary">
              <div className="flex items-start gap-3">
                <div className="w-9 h-9 rounded-lg bg-info-100 flex items-center justify-center shrink-0 mt-0.5">
                  <FileText className="w-4 h-4 text-info-700" />
                </div>
                <div>
                  <p className="font-medium text-heading">Report card publication notification</p>
                  <p className="text-sm text-heading mt-0.5">
            ...
            <div className="flex items-start justify-between gap-4 p-4 rounded-lg bg-bg-tertiary">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-medium text-heading">Term start notification</p>
                  <p className="text-sm text-heading mt-0.5">
