...
export function CommandPalette() {
  ...
  const search = useCallback(
    async (q: string) => {
      ...
      const studentResults: SearchResult[] = (students || []).map((s: { id: string; full_name: string; admission_number: string | null }) => ({
        type: "student" as const,
        title: s.full_name,
        subtitle: s.admission_number ?? undefined,
      ...
      const classResults: SearchResult[] = (classes || []).map((c: { id: string; name: string }) => ({
        type: "class" as const,
        title: c.name,
