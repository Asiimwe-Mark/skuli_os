...
export async function GET(request: NextRequest) {
  try {
    ...
    if (status) query = query.eq("subscription_status", status as Database["public"]["Enums"]["subscription_status"]);
    if (plan) query = query.eq("subscription_plan", plan as Database["public"]["Enums"]["subscription_plan"]);
    if (search) {
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const { data: school, error } = await ctx.supabase
      .from("schools")
      .update(allowedFields as Database["public"]["Tables"]["schools"]["Update"])
      .eq("id", id)
      .select()
