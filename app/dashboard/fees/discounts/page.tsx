...
export default function DiscountsPage() {
  ...
  const { data: discounts = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: discountsData, error } = await supabase
        .from("fee_discounts")
        .select("id, school_id, name, discount_type, value, description, is_active, created_at")
  ...
  const { data: studentsForDiscount = [], isLoading: studentsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select(`
          *,
          student:students(full_name, current_class:classes(name))
        `)
      ...
      return ((data || []) as any[]).map((sd) => {
        // PostgREST may return joins as objects or arrays; normalize.
        const s = Array.isArray(sd.student) ? sd.student[0] : sd.student;
        const cls = Array.isArray(s?.current_class) ? s.current_class[0] : s?.current_class;
        return {
          ...sd,
          student: s,
          student_name: s?.full_name || "Unknown",
          student_class: cls?.name || "N/A",
        };
  ...
  // ?"EUR?"EUR Form helpers ?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR?"EUR
  ...
  const upsertMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      if (school?.id) {
        invalidate.discountChanged(queryClient, school.id);
        // The student-fee-status view is a downstream view used by
        // the parent portal — keep refreshing it specifically.
        queryClient.invalidateQueries({ queryKey: queryKeys.feeStatements(school.id) });
      } else {
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-bg-tertiary border-b border-border">
                  <th className="px-4 py-3 text-left text-xs font-medium text-disabled uppercase tracking-wider">
                    Name
                  </th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-disabled uppercase tracking-wider">
                    Type
                  </th>
                  <th className="px-4 py-3 text-right text-xs font-medium text-disabled uppercase tracking-wider">
                    Value
                  </th>
                  <th className="px-4 py-3 text-right text-xs font-medium text-disabled uppercase tracking-wider">
                    Max Amount
                  ...
                  <th className="px-4 py-3 text-center text-xs font-medium text-disabled uppercase tracking-wider">
                    Students
                  </th>
                  <th className="px-4 py-3 text-right text-xs font-medium text-disabled uppercase tracking-wider">
                    Actions
                ...
                <AnimatePresence>
                  {discounts.map((item) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-sm text-right text-disabled">
                        {formatMaxAmount(item)}
                      </td>
                      <td className="px-4 py-3 text-sm">
                        {item.is_recurring ? (
                          <Badge className="bg-success-50 text-success-700 border-success-100">
                            Yes
                          </Badge>
                        ) : (
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              ...
              />
              {formErrors.name && (
                <p className="text-xs text-danger-600">{formErrors.name}</p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              {formErrors.discount_type && (
                <p className="text-xs text-danger-600">{formErrors.discount_type}</p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              />
              {formErrors.value && (
                <p className="text-xs text-danger-600">{formErrors.value}</p>
              )}
            ...
            {formType === "percentage" && (
              <div className="space-y-2">
                ...
                {formErrors.max_amount && (
                  <p className="text-xs text-danger-600">{formErrors.max_amount}</p>
                )}
                <p className="text-xs text-muted">
          ...
          <div className="mt-6 space-y-3">
            {studentsLoading ? (
              ...
              studentsForDiscount.map((sd) => (
                <div
                  key={sd.id}
                  className="flex items-center justify-between bg-bg-tertiary border border-border rounded-lg px-4 py-3"
                >
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    className="text-danger-600 hover:text-danger-700 ml-2 shrink-0"
