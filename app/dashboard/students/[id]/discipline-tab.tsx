...
export function DisciplineTab({ studentId, schoolId }: DisciplineTabProps) {
  ...
  const recordsQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data.records || []) as DisciplineRecord[];
    },
  });
  ...
  async function onSubmit(data: FormData) {
    try {
      ...
      setDialogOpen(false);
      form.reset();
      queryClient.invalidateQueries({ queryKey: ['discipline', schoolId, studentId] });
    } catch (error) {
      toast({
        title: 'Error',
  ...
  return (
    <div className="space-y-4">
      ...
      {records.length === 0 ? (
        <div className="text-center py-12 border rounded-lg bg-bg-tertiary">
              ...
              <TableRow key={record.id}>
                ...
                <TableCell>
                  {record.parent_notified ? (
                    ...
                    <Badge variant="outline" className="text-warning-600 border-border">
                      Not Notified
                    </Badge>
                  )}
                </TableCell>
