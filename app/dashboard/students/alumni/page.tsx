...
export default function AlumniPage() {
  ...
  const alumniQuery = useQuery({
    queryKey: ['alumni', school?.id],
    enabled: !!school?.id,
    queryFn: async () => {
      const res = await fetch('/api/students/alumni?limit=200');
      const result = await res.json();
      if (!result.success) {
        throw new Error(result.error || 'Failed to load alumni');
      }
      return (result.data.alumni || []) as Alumni[];
    },
  });

  const alumni = alumniQuery.data ?? [];
  const loading = alumniQuery.isLoading;

  if (alumniQuery.isError) {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (!result.success) {
        ...
        queryClient.invalidateQueries({ queryKey: ['alumni', school?.id] });
      }
    } catch {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted">Manage graduated students</p>
        </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="text-left p-4 text-sm font-medium text-muted">Admission #</th>
                    <th className="text-left p-4 text-sm font-medium text-muted">Year</th>
                    <th className="text-left p-4 text-sm font-medium text-muted">Last Class</th>
                    ...
                    <th className="text-left p-4 text-sm font-medium text-muted">Profession</th>
                  </tr>
                </thead>
                <tbody>
                  {filtered.map(alumnus => (
                    <tr key={alumnus.id} className="border-b hover:bg-card-hover">
                      <td className="p-4 font-medium">{alumnus.first_name} {alumnus.last_name}</td>
                      <td className="p-4 text-muted">{alumnus.admission_number || '-'}</td>
                      <td className="p-4">
                        <Badge variant="outline">{alumnus.graduation_year}</Badge>
                      </td>
                      <td className="p-4 text-muted">{alumnus.last_class || '-'}</td>
                      <td className="p-4 text-muted">{alumnus.phone || '-'}</td>
                      <td className="p-4 text-muted">{alumnus.email || '-'}</td>
                      <td className="p-4 text-muted">{alumnus.profession || '-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
