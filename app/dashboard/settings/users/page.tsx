    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted text-sm mt-1">Manage who has access to your school dashboard</p>
        </div>
              ...
              <TableBody>
                {users.map((user) => (
                  <TableRow key={user.id}>
                    <TableCell>
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-bg-tertiary flex items-center justify-center">
                          <UserCircle className="w-5 h-5 text-heading" />
                        </div>
                        <div>
                          <p className="font-medium text-sm">{user.full_name}</p>
                          <p className="text-xs text-heading">{user.id.slice(0, 8)}...</p>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-sm text-heading">{formatRelativeTime(user.updated_at)}</TableCell>
                    <TableCell>
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Mail className="w-5 h-5 text-secondary" />Invite User
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit((d) => inviteMutation.mutate(d))} className="space-y-4">
            ...
            <div className="space-y-2">
              <Label>Email</Label>
              <Input {...register("email")} type="email" placeholder="john@school.com" invalid={!!errors.email} />
              {errors.email && <p className="text-xs text-secondary">{errors.email.message}</p>}
            </div>
            <div className="space-y-2">
              <Label>Role</Label>
              <select {...register("role")} className="w-full h-10 px-3 rounded-lg bg-bg-tertiary border border-border text-heading text-sm">
              ...
              {errors.role && <p className="text-xs text-secondary">{errors.role.message}</p>}
            </div>
            <DialogFooter>
