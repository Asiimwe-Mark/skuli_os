...
export default function StaffDirectoryPage() {
  ...
  const { data: staffList = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("staff")
        // Note: `is_deleted` filter matches the API list endpoint so a
        // soft-deleted staff row never reappears in the directory.
        // (The query was previously unfiltered here, which let the API
        // delete-via-archive flow leave rows in place that the
        // directory should not show.)
        .select("id, school_id, employee_number, photo_url, full_name, role_title, national_id, nssf_number, bank_name, bank_account, basic_salary, hire_date, is_active, created_at")
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      // Audit (Bug #4 + #5): scope the cascade to this school.
      if (school?.id) {
        invalidate.staffChanged(queryClient, school.id);
        // Payroll uses a separate "staff-active" key — keep refreshing it
        // so a newly added teacher shows up in the payroll grid.
        queryClient.invalidateQueries({ queryKey: queryKeys.staff(school.id) });
      } else {
  ...
  const openCreate = () => {
    // Audit 5.13: the form resets correctly here, but the
    // hire_date was being computed via toISOString() (UTC). Use
    // the local YYYY-MM-DD helper for consistency with the
    // other date inputs and to avoid the off-by-one day bug
    // for users east of UTC.
    reset({ full_name: "", role_title: "", national_id: "", bank_name: "", bank_account: "", nssf_number: "", basic_salary: 0, hire_date: todayLocalISODate(), is_active: true });
    setPhotoUrl(null);
    setEditingStaff(null);
    setDialogOpen(true);
  };
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted text-sm mt-1">Manage your school staff and roles</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {staffList.map((staff, i) => (
            ...
            >
              <Card className="bg-card transition-all group">
                <CardContent className="p-5">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-full bg-bg-tertiary flex items-center justify-center overflow-hidden">
                        {staff.photo_url ? (
                          ...
                          <UserCircle className="w-7 h-7 text-heading" />
                        )}
                      </div>
                      <div>
                  ...
                  <div className="mt-4 space-y-2">
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-heading">Employee No.</span>
                      <span className="font-mono text-xs">{staff.employee_number}</span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-heading">Basic Salary</span>
                      <span className="font-medium">{formatUGX(staff.basic_salary ?? 0)}</span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-heading">Hire Date</span>
                  ...
                  <div className="mt-3 pt-3 border-t border-border flex items-center justify-between">
                    ...
                    {staff.nssf_number && (
                      <span className="text-[10px] text-heading">NSSF: {staff.nssf_number}</span>
                    )}
                  </div>
                </CardContent>
              </Card>
        ...
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          ...
          {editingStaff && isTeacher(editingStaff) && (
            <div className="flex gap-1 border-b pb-2">
              <button
                type="button"
                className={`px-3 py-1.5 text-sm rounded-t ${activeTab === 'details' ? 'bg-warning-100 text-warning-700 font-medium' : 'text-text-muted hover:text-text-heading'}`}
              ...
              <button
                type="button"
                className={`px-3 py-1.5 text-sm rounded-t ${activeTab === 'assignments' ? 'bg-warning-100 text-warning-700 font-medium' : 'text-text-muted hover:text-text-heading'}`}
          ...
          {activeTab === 'assignments' && editingStaff ? (
            <div className="space-y-4">
              ...
              {assignments.length === 0 ? (
                <p className="text-sm text-heading text-center py-4">No assignments yet</p>
              ) : (
                <div className="space-y-2">
                  {assignments.map((a, i) => {
                    ...
                    return (
                      <div key={i} className="flex items-center justify-between p-2 bg-bg-tertiary rounded text-sm">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            variant="ghost"
                            size="sm"
                            // Audit 10.10: destructive actions should
                            // confirm. A misclick here would silently
                            // unassign the teacher from a class+subject
                            // (a soft delete, but still surprising).
                            // window.confirm is enough for now — the
                            // team already uses it elsewhere and the
                            // confirmation copy is teacher-specific.
                            onClick={async () => {
                              const cls = classes.find((c: { id: string; name: string }) => c.id === a.class_id);
                              const sub = subjects.find((s: { id: string; name: string }) => s.id === a.subject_id);
                              const ok = window.confirm(
                                `Remove ${editingStaff.full_name}'s assignment to ${cls?.name ?? "this class"}${
                                  a.subject_id ? ` (${sub?.name ?? "subject"})` : ""
                                }?`,
                              );
                              if (!ok) return;
              ...
              {/* Add assignment */}
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 p-3 bg-bg-tertiary rounded-lg">
                <select
                  value={newAssignment.class_id}
                  onChange={(e) => setNewAssignment((p) => ({ ...p, class_id: e.target.value }))}
                  className="flex-1 rounded-lg border border-border bg-bg-tertiary px-2 py-1.5 text-sm text-heading"
                >
          ...
          <form onSubmit={handleSubmit((data) => saveMutation.mutate(data as StaffFormData))} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input {...register("full_name")} placeholder="John Doe" invalid={!!errors.full_name} />
                {errors.full_name && <p className="text-xs text-secondary">{errors.full_name.message}</p>}
              </div>
              <div className="space-y-2">
                <Label>Role / Title</Label>
                <Input {...register("role_title")} placeholder="e.g. Teacher, Bursar" invalid={!!errors.role_title} />
                {errors.role_title && <p className="text-xs text-secondary">{errors.role_title.message}</p>}
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              />
              {errors.basic_salary && <p className="text-xs text-secondary">{errors.basic_salary.message}</p>}
            </div>
