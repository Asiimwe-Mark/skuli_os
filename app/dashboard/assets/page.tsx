...
export default function AssetsPage() {
  ...
  const { data: staff } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch('/api/staff?lite=true');
      const json = await res.json();
      if (!json.success) throw new Error(json.error);
      return json.data;
    },
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-foreground/60 mt-1">Track school assets, maintenance, and depreciation</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="border-border-subtle bg-surface">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-foreground/60">Total Assets</p>
        ...
        <Card className="border-border-subtle bg-surface">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold">{formatUGX(totalValue)}</p>
            </div>
          </CardContent>
        </Card>
        <Card className="border-border-subtle bg-surface">
        ...
        <Card className="border-border-subtle bg-surface">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-foreground/60">Maintenance Due</p>
              <p className="text-xl font-bold text-rose-400">{maintenanceDue.length}</p>
            </div>
          </CardContent>
        </Card>
      </div>
          ...
          <CardContent className="p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {maintenanceDue.slice(0, 4).map(m => (
                <div key={m.id} className="flex items-center justify-between text-sm bg-amber-500/10 rounded-lg p-3">
                  <div>
                    <p className="font-medium">{m.assets?.name}</p>
                    <p className="text-xs text-foreground/60">{m.assets?.asset_code}</p>
                  </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          ...
                          {asset.category && <p className="text-xs text-foreground/50">{asset.category}</p>}
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3 text-sm text-foreground/70">{asset.location || '—'}</td>
                        <td className="px-4 py-3 text-sm text-foreground/70">{asset.users?.full_name || '—'}</td>
                        <td className="px-4 py-3 text-sm text-right">{asset.purchase_price ? formatUGX(asset.purchase_price) : '—'}</td>
                        <td className="px-4 py-3 text-sm text-right">
                          <span className="font-medium">{formatUGX(asset.current_value ?? depreciatedValue)}</span>
                        ...
                        <td className="px-4 py-3 text-right">
                          <div className="flex justify-end gap-1">
                            ...
                            <Button variant="ghost" size="sm" className="text-rose-400 hover:text-rose-300" onClick={() => {
          ...
          {selectedAsset && (
            <div className="space-y-4 py-4">
              <div className="bg-navy-800 rounded-lg p-3">
                ...
                <p className="text-xs text-foreground/50">{selectedAsset.asset_code}</p>
              </div>
              <div>
