  ...
  return (
    <div className="p-8 max-w-[1200px] mx-auto">
      <motion.div {...fadeIn} className="mb-8">
        <h1 className="text-3xl font-bold text-navy mb-2">Custom Report Builder</h1>
        <p className="text-gray-500">Generate custom reports by selecting data sources, columns, and filters.</p>
      </motion.div>
            ...
            <button
              ...
              className={cn(
                ...
                step === s.n
                  ...
                  : 'bg-surface border border-border-subtle text-foreground/40'
              )}
            >
              {s.n}
            </button>
            <span className={cn('text-sm', step === s.n ? 'text-foreground font-medium' : 'text-foreground/50')}>
              {s.label}
            </span>
            {i < 2 && <ChevronRight className="w-4 h-4 text-foreground/20 mx-1" />}
          </div>
        ))}
      </div>
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-foreground/50 mt-1">{source.description}</p>
                </div>
              </div>
            </button>
          );
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card className="bg-surface border-border-subtle">
          ...
          <CardContent className="space-y-2">
            ...
            {available.length === 0 && (
              <p className="text-sm text-foreground/40 py-4 text-center">All fields selected</p>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="bg-surface border-border-subtle">
          ...
          <CardContent className="space-y-1.5">
            ...
            {selected.length === 0 && (
              <p className="text-sm text-foreground/40 py-4 text-center">Select fields from the left panel</p>
            )}
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent className="space-y-3">
          {filters.map((filter, idx) => (
            <div key={idx} className="flex items-center gap-2">
              ...
              <button onClick={() => removeFilter(idx)} className="p-2 hover:bg-rose-500/20 rounded text-rose-400">
                <Trash2 className="w-4 h-4" />
              </button>
            </div>
          ))}
          {filters.length === 0 && (
            <p className="text-sm text-foreground/40 py-2">No filters applied — all records will be included</p>
          )}
        </CardContent>
      </Card>
...
function Step3Preview({ config, sourceLabel, onBack }: { config: ReportConfig; sourceLabel: string; onBack: () => void }) {
  ...
  // Fetch preview data
  useEffect(() => {
    if (!school?.id || !config.source) return;
    setLoading(true);

    const fetchPreview = async () => {
      // Build query based on source
      let query: any;
      const baseFilters = { school_id: school.id, is_deleted: false };
      ...
      <Card className="bg-surface border-border-subtle">
        ...
        <CardContent>
          {loading ? (
            ...
            <p className="text-foreground/40 text-sm py-8 text-center">No data found matching your criteria</p>
          ) : (
                ...
                <thead>
                  <tr className="border-b border-border-subtle">
                    {selectedFields.map((f) => (
                      <th key={f.key} className="text-left py-2 px-3 text-foreground/50 font-medium whitespace-nowrap">{f.label}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {previewData.map((row, i) => (
                    <tr key={i} className="border-b border-border-subtle/50 hover:bg-white/[0.02]">
        ...
        <Card className="bg-surface border-border-subtle">
          ...
          <CardContent className="space-y-2">
            {savedReports.map((saved, idx) => (
              <div key={idx} className="flex items-center gap-3 p-2 rounded-md bg-white/[0.02] border border-border-subtle/50">
                ...
                <button onClick={() => handleDeleteSaved(idx)} className="p-1 hover:bg-rose-500/20 rounded text-rose-400">
                  <Trash2 className="w-3 h-3" />
                </button>
              </div>
            ))}
