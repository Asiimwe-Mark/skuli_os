...
const statusConfig: Record<string, { label: string; variant: "default" | "secondary" | "destructive" | "outline"; color: string }> = {
  not_started: { label: "Not Started", variant: "secondary", color: "text-text-muted" },
  draft: { label: "Draft", variant: "outline", color: "text-text-muted" },
  submitted: { label: "Submitted", variant: "default", color: "text-warning-700" },
  approved: { label: "Approved", variant: "default", color: "text-success-700" },
  rejected: { label: "Rejected", variant: "destructive", color: "text-danger-700" },
};
  ...
  const approveMutation = useMutation({
    mutationFn: async (group: MarkGroup) => {
      const { error } = await supabase
        ...
        .eq("exam_type", group.exam_type as ExamType)
    ...
    onSuccess: () => {
      ...
      // Cross-page: keep the marks entry sheet's per-row review_status in sync.
      ...
      <div className="flex flex-wrap gap-2">
        {["all", "not_started", "draft", "submitted", "approved", "rejected"].map((s) => (
          <Button
            ...
            className={cn(
              "text-xs",
              statusFilter === s && s === "submitted" && "bg-warning-100 text-warning-700 border-warning-500 hover:bg-warning-100",
              statusFilter === s && s === "approved" && "bg-success-100 text-success-700 border-success-500 hover:bg-success-100",
              statusFilter === s && s === "rejected" && "bg-danger-100 text-danger-700 border-danger-500 hover:bg-danger-100"
            )}
          >
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-12">
              <ClipboardCheck className="w-12 h-12 mx-auto mb-3 text-heading" />
              <p className="text-heading font-medium">No mark submissions found</p>
              <p className="text-heading text-sm mt-1">
                {statusFilter !== "all"
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b">
                    <th className="text-left py-3 px-2 font-medium text-heading">Class</th>
                    <th className="text-left py-3 px-2 font-medium text-heading">Subject</th>
                    <th className="text-left py-3 px-2 font-medium text-heading">Teacher</th>
                    <th className="text-left py-3 px-2 font-medium text-heading">Exam Type</th>
                    <th className="text-center py-3 px-2 font-medium text-heading">Students</th>
                    <th className="text-center py-3 px-2 font-medium text-heading">Status</th>
                    <th className="text-left py-3 px-2 font-medium text-heading">Last Updated</th>
                    <th className="text-right py-3 px-2 font-medium text-heading">Actions</th>
                  </tr>
                </thead>
                <tbody>
                      ...
                      >
                        ...
                        <td className="py-3 px-2 text-center">
                          ...
                          {group.review_comment && (
                            <p className="text-[10px] text-heading mt-1 truncate max-w-[120px]" title={group.review_comment}>
                              {group.review_comment}
                            </p>
                          )}
                        </td>
                        <td className="py-3 px-2 text-heading text-xs">
                          ...
                          <div className="flex items-center justify-end gap-1">
                            {group.review_status === "submitted" && (
                              <>
                                <Button
                                  size="sm"
                                  variant="ghost"
                                  className="text-success-700 hover:text-success-700 hover:bg-success-100"
                                ...
                                <Button
                                  size="sm"
                                  variant="ghost"
                                  className="text-danger-700 hover:text-danger-700 hover:bg-danger-100"
                            ...
                            {group.review_status === "approved" && (
                              <Button
                                size="sm"
                                variant="ghost"
                                className="text-warning-700 hover:text-warning-700 hover:bg-warning-100"
                            ...
                            {(group.review_status === "not_started" || group.review_status === "draft") && (
                              <span className="text-xs text-heading">-</span>
                            )}
                          </div>
                        </td>
                      </tr>
