...
const CATEGORY_COLORS: Record<string, string> = {
  fees: "bg-success-100 text-success-700",
  academics: "bg-info-100 text-info-700",
  attendance: "bg-warning-100 text-warning-700",
  general: "bg-bg-tertiary text-text-heading",
};
...
export default function TemplatesPage() {
  ...
  const { data: templates = [], isLoading } = useQuery({
    queryKey: ["communication-templates", school?.id],
  ...
  const handleSave = async () => {
    ...
    setDialogOpen(false);
    setSaving(false);
    queryClient.invalidateQueries({ queryKey: ["communication-templates", school?.id] });
  };
                  ...
                  <div className="flex flex-wrap gap-1 mt-3">
                    {(template.variables ?? []).map((v) => (
                      <Badge
                        key={v}
                        variant="outline"
                        className="text-[10px] text-warning-600 border-warning-50"
                      >
                        {`{${v}}`}
                      </Badge>
                    ))}
              ...
              <div className="flex flex-wrap gap-1 mb-2">
                {[
                  ...
                  <button
                    ...
                    className="px-2 py-0.5 rounded bg-bg-tertiary text-xs text-warning-600 hover:bg-card-hover"
                  >
                    {`{${v}}`}
                  </button>
                ))}
