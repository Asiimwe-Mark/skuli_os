...
import type { TeacherAssignment, ExamType } from '@/types';
...
type Assignment = TeacherAssignment;
...
export default function TeacherMarksPage() {
  ...
  useEffect(() => {
    ...
    type ExistingMark = Pick<Tables<"marks">, "id" | "student_id" | "score" | "remarks">;
    type StudentEnrollment = { student_id: string; students: { id: string; full_name: string; admission_number: string } | null };
    const marksMap = new Map<string, ExistingMark>(
      existingMarks.map((m) => [m.student_id, m])
    );
    setMarks(
      (students as StudentEnrollment[]).map((e) => {
  ...
  const autoSaveMark = useCallback(
    async (mark: MarkEntry) => {
      ...
      try {
        const { data: { user } } = await supabase.auth.getUser();
        const markData: Database["public"]["Tables"]["marks"]["Insert"] = {
          school_id: school.id,
  ...
  return (
    <div className="p-4 sm:p-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-secondary mb-2">Marks Entry</h1>
        <p className="text-muted">Enter and manage marks for your assigned classes.</p>
      </div>
          ...
          <div className="border rounded-lg overflow-hidden">
            <table className="w-full">
              <thead className="bg-bg-tertiary">
                <tr>
                  <th className="px-4 py-3 text-left text-sm font-semibold text-heading">
                    Student Name
                  </th>
                  <th className="px-4 py-3 text-left text-sm font-semibold text-heading">
                    Admission No.
              ...
              <tbody className="divide-y">
                {marks.map((mark, index) => {
                  ...
                  return (
                    <tr key={mark.student_id} className="hover:bg-card-hover">
                      <td className="px-4 py-3 text-sm font-medium">{mark.student_name}</td>
                      <td className="px-4 py-3 text-sm text-muted">{mark.admission_number}</td>
                      <td className="px-4 py-3">
                        <Input
                          ...
                          className={cn(
                            'w-24',
                            isError && 'border-border focus-visible:ring-border'
                          )}
                          disabled={isSaving}
                        />
                      </td>
